        ...
        <>
          {role === 'admin' && (
            <li className={isMobile ? 'w-full mb-2' : ''}>
              <Link
                href="/pageadmin"
                className={isMobile
                  ? 'block text-lg font-medium bg-blue-900 text-white hover:bg-blue-600 rounded-lg text-center py-2'
                  : 'text-white rounded-lg px-5 py-2 font-semibold bg-blue-900 hover:bg-blue-700 text-lg'}
              >
                Dashboard
              </Link>
            </li>
          ...
          <li className={isMobile ? 'w-full' : ''}>
            <button
              onClick={logout}
              className={isMobile
                ? 'block text-lg font-medium bg-red-600 text-white hover:bg-red-700 rounded-lg text-center py-2 w-full'
                : 'text-white hover:bg-red-700 px-5 bg-red-600 rounded-lg py-2 font-semibold text-lg cursor-pointer'}
            >
              Sair
            </button>
          </li>
    ...
    <div className="absolute left-5 top-[-10px] sm:top-0 sm:left-0 w-[calc(100%-2rem)] sm:w-auto pointer-events-none z-[5000]">
      <div className="relative w-full max-w-6xl sm:p-4 pointer-events-auto">
        ...
        <div
          ...
          `}
        >
          <header className="w-full bg-white px-6 py-2 shadow-lg flex flex-col sm:flex-row items-center justify-between rounded-2xl">
              ...
              <ul className="w-full bg-white text-black shadow-lg flex flex-col items-start p-4 sm:hidden rounded-2xl border border-gray-200 mt-2">
                <li className="mb-2 w-full">
                  <Link
                    ...
                    className={`block text-lg font-medium ${
                      pathname === '/' ? 'border-b-4 border-blue-900' : ''
                    }`}
                  >
                    Início
                  </Link>
                ...
                <li className="mb-2 w-full">
                  <Link
                    href="/sobre"
                    className={`block text-lg font-medium ${
                      pathname === '/sobre' ? 'border-b-4 border-blue-900' : ''
                    }`}
                  >
                    Sobre
                  </Link>
                ...
                <li className="mb-4 w-full">
                  <Link
                    href="/contato"
                    className={`block text-lg font-medium ${
                      pathname === '/contato' ? 'border-b-4 border-blue-900' : ''
                    }`}
                  >
                    Contato
                  </Link>
            ...
            <ul className="hidden sm:flex space-x-6 items-center ml-auto">
              <li>
                <Link
                  href="/"
                  className={`text-blue-900 font-medium ml-4 text-lg pb-1 ${
                    pathname === '/' ? 'border-b-4 border-blue-900' : ''
                  }`}
                >
                  Início
              ...
              <li>
                <Link
                  href="/sobre"
                  className={`text-blue-900 font-medium text-lg pb-1 ${
                    pathname === '/sobre' ? 'border-b-4 border-blue-900' : ''
                  }`}
                >
                  Sobre
