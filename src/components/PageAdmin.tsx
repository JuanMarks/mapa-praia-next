...
const PageAdmin = () => {
  ...
  useEffect(() => {
    const fetchPontos = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error('Falha ao buscar os pontos.');
        }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Tem certeza que deseja apagar este ponto?')) {
      try {
        const res = await fetch(`http://25.20.79.62:3003/pontos/${id}`, {
          method: 'DELETE',
        });

        if (!res.ok) {
          throw new Error('Falha ao apagar o ponto.');
        }
                ...
                {pontos.map((ponto) => (
                  <tr key={ponto.id}>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <p className="text-gray-900 whitespace-no-wrap">{ponto.nome}</p>
                    </td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <p className="text-gray-900 whitespace-no-wrap max-w-md truncate">{ponto.descricao}</p>
                    </td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <button
