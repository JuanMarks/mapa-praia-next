...
import { useState, useEffect, FC, useMemo } from 'react'; // Importe o useMemo
...
// 1. Mapeamento dos tipos para os botões. Use os valores exatos do seu backend.
const CATEGORIAS = {
    'Todos': 'todos',
...
const CarrosselHorizontal: FC = () => {
    ...
    // 3. Lógica de filtragem e ordenação usando useMemo para otimização
    const pontosFiltrados = useMemo(() => {
        return pontos
            .filter(ponto => {
                ...
                // Compara o tipo do ponto com o filtro ativo (ignorando maiúsculas/minúsculas)
                return ponto.type?.toLowerCase() === filtroAtivo.toLowerCase();
            })
            .sort((a, b) => {
                // Ordena pelos melhores avaliados (maior rating primeiro)
                // O `|| 0` trata casos onde a avaliação pode ser nula
    ...
    const getImagemDoPonto = (ponto: PontoTuristico): string => {
        if (ponto.photos && ponto.photos.length > 0) {
            return ponto.photos[0]; // Pega a primeira imagem para consistência
        }
        return '/images/img1.jpeg'; // Imagem padrão
    };
                                        ...
                                        {Object.entries(CATEGORIAS).map(([nome, tipo]) => (
                                            <button
                                                ...
                                                className={`px-4 py-1.5 border rounded-full text-sm font-semibold transition-all duration-200 ${
                                                    filtroAtivo === tipo
                                                        ? 'bg-blue-600 text-white border-blue-600' // Estilo do botão ativo
